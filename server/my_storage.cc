...
public:
  ...
  virtual result_t add_user(const string &user, const string &pass) {
    ...
    Quotas *q = new Quotas();
    q->uploads = quota_factory(up_quota, quota_dur);
    q->downloads = quota_factory(down_quota, quota_dur);
    q->requests = quota_factory(req_quota, quota_dur);
    quota_table->insert(user, q, [](){});
    ...
    quota_table->do_with(user, [&](Quotas *q) {
      if (q->requests->check_add(1)) {
        if (q->uploads->check_add(val.size())) {
          bool ret = kv_store->insert(key, val, [&](){
            log_sv(storage_file, KVENTRY, key, val);
            mru->insert(key);
          });
          ret? res = {true, RES_OK, {}} : res = {false, RES_ERR_KEY, {}};
        } else res = {false, RES_ERR_QUOTA_UP, {}};
      } else res = {false, RES_ERR_QUOTA_REQ, {}};
    });
    return res;
  };
  ...
  virtual result_t kv_get(const string &user, const string &pass,
    ...
    quota_table->do_with(user, [&](Quotas *q) {
      if (q->requests->check_add(1)) {
        if (!kv_store->do_with_readonly(key, [&](const vector<uint8_t> &val) {
          vec = val;
        })) {
          res = {false, RES_ERR_KEY, {}};
        } else {
          if (q->downloads->check_add(vec.size())) {
            mru->insert(key);
            res = {true, RES_OK, vec};
          } else res = {false, RES_ERR_QUOTA_DOWN, {}};
        }
      } else res = {false, RES_ERR_QUOTA_REQ, {}};
    });
    return res;
  ...
  virtual result_t kv_upsert(const string &user, const string &pass,
    ...
    quota_table->do_with(user, [&](Quotas *q) {
      if (q->requests->check_add(1)) {
        if (q->uploads->check_add(val.size())) {
          bool ret = kv_store->upsert(key, val, [&](){
            log_sv(storage_file, KVENTRY, key, val);
            mru->insert(key);
          }, [&](){
          ...
          ret? res = {true, RES_OKINS, {}} : res = {true, RES_OKUPD, {}};
        } else res = {false, RES_ERR_QUOTA_UP, {}};
      } else res = {false, RES_ERR_QUOTA_REQ, {}};
    });
    return res;
  ...
  virtual result_t kv_all(const string &user, const string &pass) {
    ...
    if (!res.succeeded)
      return res;
    std::vector<uint8_t> ret_keys;
    quota_table->do_with(user, [&](Quotas *q) {
      if (q->requests->check_add(1)) {
          kv_store->do_all_readonly([&](std::string key, const std::vector<uint8_t> &) {
            std::string copy(key);
            copy += '\n';
            std::vector<uint8_t>vec(copy.begin(), copy.end());
            ret_keys.insert(ret_keys.end(), vec.begin(), vec.end());
          }, [&](){
            bool ret = q->downloads->check_add(ret_keys.size());
            ret? res = {true, RES_OK, ret_keys} : res = {false, RES_ERR_QUOTA_DOWN, {}};
          });
        } else {
          res = {false, RES_ERR_QUOTA_REQ, {}};
        }
      });
    return res;
  };
  ...
  virtual result_t kv_top(const string &user, const string &pass) {
    ...
    quota_table->do_with(user, [&](Quotas *q) {
      if (q->requests->check_add(1)) {
        std::string MRUCache = mru->get();
        if (q->downloads->check_add(MRUCache.size())) {
          std::vector<uint8_t> vec(MRUCache.begin(), MRUCache.end());
          if (vec.size() <= 0) res = {false, RES_ERR_NO_DATA, {}};
          else res = {true, RES_OK, vec};
        } else res = {false, RES_ERR_QUOTA_DOWN, {}};
      } else res = {false, RES_ERR_QUOTA_REQ, {}};
    });
    return res;
